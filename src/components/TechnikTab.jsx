...
export default function TechnikTab() {
  ...
  async function addFromTemplate(tmpl) {
    ...
    let nr
    const { data: d1, error: e1 } = await supabase.from('technik_routines').insert({ ...routineData, position: pos }).select().single()
    if (e1) {
      const { data: d2, error: e2 } = await supabase.from('technik_routines').insert(routineData).select().single()
      if (e2) return
      nr = d2
    } else {
      nr = { ...d1, position: pos }
    ...
    if (tmplSteps) {
      ...
      const { data: ns } = await supabase.from('technik_steps').insert(sd).select()
